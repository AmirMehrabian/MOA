...
def init_channel_solver(scene, config_dict: dict):
    ...
    solver = PathSolver()
    channel_dict = {"solver": solver,
                    "rx": rx,
                    "tx_friendly_idx": tx_friendly_idx,
                    "tx_jammer_idx": tx_jammer_idx}
...
def get_sionna_channels(scene,
    ...
    rx_vel = config_dict.get("rx_velocity")
    rx_height = config_dict.get("rx_height", 1.5)
    ...
    taps = paths.taps(
        ...
        sampling_frequency=None,  # one sample per symbol
        num_time_steps=T,
        out_type="numpy",
    )
